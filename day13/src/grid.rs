...
impl<'a, T> Iterator for GridRowIter<'a, T> {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        if n as isize > self.grid.height {
            None
        } else {
            let begin = n * self.grid.width as usize;
            let end = begin + self.grid.width as usize;
            Some(&self.grid.data[begin..end])
        }
    }
}
...
impl<'a, T> Iterator for GridColIter<'a, T> {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        if n as isize > self.grid.width {
            None
        } else {
            Some(self.grid.data[n..].iter().step_by(self.grid.width as usize))
        }
    }
}
